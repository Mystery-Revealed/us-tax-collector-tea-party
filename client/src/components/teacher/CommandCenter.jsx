...
// → approve names → watch live status, one class-wide accuracy average, AND a
// per-act accuracy row (spec §9 — helps a teacher spot which act to reteach; the
// Tea Act is the low bar by design) → download the PDF → End Session.
// Tax Collector vs. Tea Party is single-class solo (no pick): students start on
// their own as soon as they're approved — no pairing, one class group.
...
const GROUP = 'class';
const TEKS = 'TEKS 8.4A, 8.20B, 8.29B';
...
export default function CommandCenter() {
  ...
  if (phase === 'gate') {
    return (
      <div className="app teacher-app">
        ...
        <p className="muted">Tax Collector vs. Tea Party · {TEKS}</p>
  ...
  const chapters = roster?.meta?.chapters || [];
  const chapterAcc = roster?.chapterAccuracy || [];
  ...
  return (
    <div className="app teacher-app">
      <header className="row">
        <div>
          <h1>Teacher Command Center</h1>
          <p className="muted">Tax Collector vs. Tea Party · session-only data · download the PDF before you end</p>
        </div>
      ...
      {pending.length > 0 && (
        <div className="card">
          ...
          <p className="muted">Approve a name and that student starts right away.</p>
          {pending.map((s) => (
      ...
      <div className="card">
        <div className="class-summary one">
          <div className="summary-stat wide">
            ...
            <div className="muted">Class accuracy — {students.length} students — {cls.count} completed</div>
          </div>
        </div>
      </div>
      ...
      {chapters.length > 0 && (
        <div className="card">
          <h2>Accuracy by act</h2>
          <p className="muted">Spot which act to reteach. The Tea Act is the trick round — a low bar there is expected.</p>
          <div className="table-wrap">
            <table>
              <thead>
                <tr><th>Act</th><th>Actions graded</th><th>Class accuracy</th></tr>
              </thead>
              <tbody>
                {chapters.map((c, i) => {
                  const acc = chapterAcc[i] || { count: 0, average: null };
                  return (
                    <tr key={i}>
                      <td>{c.title} <span className="muted">({c.year})</span></td>
      ...
      <div className="card">
        ...
        <div className="btn-row">
          <button className="btn" onClick={() => downloadPdf(roster, cls, chapters, chapterAcc)}>⬇ Download PDF report</button>
        </div>
...
function downloadPdf(roster, cls, chapters, chapterAcc) {
  if (!roster) return;
  ...
  doc.setFontSize(16);
  doc.text('Tax Collector vs. Tea Party — Session Report', 14, 16);
  doc.setFontSize(10);
  doc.setTextColor(90);
  ...
  if (chapters.length) {
    ...
    doc.text('Accuracy by act', 14, nextY);
    const actRows = chapters.map((c, i) => {
      const acc = chapterAcc[i] || { count: 0, average: null };
      return [`${c.title} (${c.year})`, String(acc.count), acc.average != null ? `${acc.average}%` : '—'];
    });
    autoTable(doc, {
      startY: nextY + 4,
      head: [['Act', 'Actions graded', 'Class accuracy']],
      body: actRows,
      styles: { fontSize: 9 },
      headStyles: { fillColor: BRAND },
    });
  ...
  doc.text('8th Grade U.S. History · Tax Collector vs. Tea Party · TEKS 8.4A, 8.20B, 8.29B', 14, doc.internal.pageSize.getHeight() - 8);

  doc.save(`tax-collector-vs-tea-party_${roster.joinCode}_${date}.pdf`);
}
