  ...
  return {
    ...
    resolve(state, side, move) {
      ...
      if (step.kind === 'multiSelect') {
        ...
        ss.actions.push({ stepIndex: ss.cursor, chapter: chapterOf(ss.cursor), kind: step.kind, verdict, points: pointsFor(verdict) });
        ss.cursor += 1;
        return {
          side,
          kind: step.kind,
      ...
      if (move.choiceIndex == null || move.choiceIndex < 0) {
        ...
        ss.actions.push({ stepIndex: ss.cursor, chapter: chapterOf(ss.cursor), kind: step.kind, verdict: 'wrong', points: 0 });
        ss.cursor += 1;
        return {
          side,
          kind: step.kind,
      ...
      ss.actions.push({ stepIndex: ss.cursor, chapter: chapterOf(ss.cursor), kind: step.kind, verdict: choice.verdict, points: pointsFor(choice.verdict) });
      ss.cursor += 1;
